...
////
  ...
  stages {
    ...
    stage('Build Docker Image') {
      steps {
        sh "cp ./gitea/Dockerfile ./repos/gitea/gitea/"
        sh "docker build -t ${IMAGE_NAME}:${VERSION_TAG} ./repos/gitea/gitea/"
      }
    }

    stage('Test Image') {
      steps {
        sh "docker run --rm ${IMAGE_NAME}:${VERSION_TAG} java -version"
      }
    }
